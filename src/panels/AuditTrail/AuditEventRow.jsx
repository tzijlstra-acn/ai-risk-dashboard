...
function TypeIcon({ type }) {
  ...
  if (type === 'incident') return <Zap className={`${classes} text-severity-critical`} />;
  if (type === 'approval') return <CheckCircle className={`${classes} text-severity-healthy`} />;
  if (type === 'validation') return <Clock className={`${classes} text-severity-low`} />;
  return <AlertTriangle className={`${classes} text-severity-medium`} />;
}
...
export function AuditEventRow({ event }) {
  ...
  return (
    <div
      className={[
        'px-4 py-4 border-b border-surface-600/50 transition-colors',
        isAuditFinding ? 'bg-severity-stale/5 border-l-2 border-l-severity-stale' : 'hover:bg-surface-700/30',
      ].join(' ')}
    >
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                ...
                {/* Audit Finding badge for >48h pending */}
                {isAuditFinding && (
